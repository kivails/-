...
def top_messages(chat_id=None, limit=10, hours=None):
    with lock, sqlite3.connect(DB_PATH) as conn:
        ...
        if chat_id and hours:
            ...
            cur.execute("SELECT cs.user_id, u.username, u.nickname, u.first_name, cs.messages FROM chat_stats cs LEFT JOIN users u ON cs.user_id=u.user_id WHERE cs.chat_id=? AND cs.last_message>=? ORDER BY cs.messages DESC LIMIT ?", (chat_id, from_date, limit))
        elif chat_id:
            cur.execute("SELECT cs.user_id, u.username, u.nickname, u.first_name, cs.messages FROM chat_stats cs LEFT JOIN users u ON cs.user_id=u.user_id WHERE cs.chat_id=? ORDER BY cs.messages DESC LIMIT ?", (chat_id, limit))
        else:
...
def add_message(user_id, chat_id):
    ...
    with lock, sqlite3.connect(DB_PATH) as conn:
        ...
        cur.execute("INSERT INTO chat_stats(chat_id, user_id, messages, last_message) VALUES(?, ?, 1, ?) ON CONFLICT(chat_id, user_id) DO UPDATE SET messages = messages + 1, last_message = ?", (chat_id, user_id, now, now))
        conn.commit()
...
def create_marriage_proposal(proposer_id, target_id):
    with lock, sqlite3.connect(DB_PATH) as conn:
        cur = conn.cursor()
        cur.execute("INSERT OR REPLACE INTO marriages_proposals(proposer_id, target_id, created_at) VALUES(?, ?, ?)", (proposer_id, target_id, datetime.now().isoformat()))
        conn.commit()
...
def create_clan(name, owner_id, description=""):
    with lock, sqlite3.connect(DB_PATH) as conn:
        cur = conn.cursor()
        try:
            cur.execute("INSERT INTO clans(name, owner_id, description, created_at) VALUES(?, ?, ?, ?)", (name, owner_id, description, datetime.now().isoformat()))
            conn.commit()
...
def add_referral(referrer_id, referred_id):
    ...
    with lock, sqlite3.connect(DB_PATH) as conn:
        ...
        if cur.fetchone():
            return False
        cur.execute("INSERT INTO referrals(referrer_id, referred_id, created_at) VALUES(?, ?, ?)", (referrer_id, referred_id, datetime.now().isoformat()))
...
def add_custom_rp(owner_id, command, emoji, action):
    with lock, sqlite3.connect(DB_PATH) as conn:
        cur = conn.cursor()
        try:
            cur.execute("INSERT INTO custom_rp(owner_id, command, emoji, action) VALUES(?, ?, ?, ?)", (owner_id, command.lower(), emoji, action))
            conn.commit()
            return True
        except sqlite3.IntegrityError:
            return False
...
def add_announcement(user_id, text):
    with lock, sqlite3.connect(DB_PATH) as conn:
        cur = conn.cursor()
        cur.execute("INSERT INTO announcements(user_id, text, created_at) VALUES(?, ?, ?)", (user_id, text, datetime.now().isoformat()))
        conn.commit()
...
def add_business(user_id, biz_key):
    with lock, sqlite3.connect(DB_PATH) as conn:
        cur = conn.cursor()
        cur.execute("INSERT INTO businesses(user_id, biz_key, purchased_at, last_collect) VALUES(?, ?, ?, ?)", (user_id, biz_key, datetime.now().isoformat(), datetime.now().isoformat()))
        conn.commit()
...
def add_deposit(user_id, amount, rate, end_at):
    with lock, sqlite3.connect(DB_PATH) as conn:
        cur = conn.cursor()
        cur.execute("INSERT INTO deposits(user_id, amount, rate, end_at, created_at) VALUES(?, ?, ?, ?, ?)", (user_id, amount, rate, end_at, datetime.now().isoformat()))
        conn.commit()
...
def create_promo(code, reward, max_activations, expires_hours=24):
    ...
    with lock, sqlite3.connect(DB_PATH) as conn:
        cur = conn.cursor()
        try:
            cur.execute("INSERT INTO promos(code, reward, activations, max_activations, expires_at, created_at) VALUES(?, ?, 0, ?, ?, ?)", (code.upper(), reward, max_activations, expires_at, datetime.now().isoformat()))
            conn.commit()
            return True
        except sqlite3.IntegrityError:
            return False
...
def use_promo(code, user_id):
    ...
    with lock, sqlite3.connect(DB_PATH) as conn:
        ...
        cur.execute("INSERT INTO promo_uses(code, user_id, used_at) VALUES(?, ?, ?)", (code, user_id, datetime.now().isoformat()))
...
def add_sponsor(chat_id, title, url, reward):
    with lock, sqlite3.connect(DB_PATH) as conn:
        cur = conn.cursor()
        cur.execute("INSERT INTO sponsors(chat_id, title, url, reward, created_at) VALUES(?, ?, ?, ?, ?)", (chat_id, title, url, reward, datetime.now().isoformat()))
        conn.commit()
...
def mark_sponsor_done(sponsor_id, user_id):
    with lock, sqlite3.connect(DB_PATH) as conn:
        cur = conn.cursor()
        try:
            cur.execute("INSERT INTO sponsor_done(sponsor_id, user_id, done_at) VALUES(?, ?, ?)", (sponsor_id, user_id, datetime.now().isoformat()))
            conn.commit()
            return True
        except sqlite3.IntegrityError:
            return False
...
def add_custom_achievement(ach_key, name, description, emoji, created_by):
    with lock, sqlite3.connect(DB_PATH) as conn:
        cur = conn.cursor()
        try:
            cur.execute("INSERT INTO custom_achievements(ach_key, name, description, emoji, created_by, created_at) VALUES(?, ?, ?, ?, ?, ?)", (ach_key, name, description, emoji, created_by, datetime.now().isoformat()))
            conn.commit()
            return True
        except sqlite3.IntegrityError:
            return False
...
def add_limited_gift(gift_key, name, price, days=5):
    ...
    with lock, sqlite3.connect(DB_PATH) as conn:
        cur = conn.cursor()
        cur.execute("INSERT INTO limited_gifts(gift_key, name, price, available_from, available_to) VALUES(?, ?, ?, ?, ?)", (gift_key, name, price, available_from, available_to))
        conn.commit()
