...
@flask_app.route('/')
def index():
    return "<html><body style='background:#1a1a2e;color:#eee;text-align:center;padding:50px;font-family:sans-serif'><h1>🤖 Бот</h1><p style='color:#4ade80;font-size:20px'>✅ Online</p></body></html>", 200
...
async def start(update, context):
    ...
    await update.message.reply_text(
        ...
        f"💠 /donate — донат\n"
        f"{divider()}\n\n"
        f"💡 Русские команды без слэша:\n"
        f"рыбалка, работа, баланс, профиль, топ,\n"
        f"монетка, кубик, брак @юзер, шип, время"
    )
...
async def help_command(update, context):
    await update.message.reply_text(
        ...
        f"📊 /stats — статистика\n"
        f"🏆 /top — топ монет\n"
        f"💎 /top_ref — топ кристаллов\n"
        f"💠 /top_donat — топ донатов\n"
        f"🎰 /top_casino — топ побед\n"
        f"🕐 /time — время в городах\n"
        f"💱 /currency — валюты\n"
        f"ℹ️ /about — о боте"
    )
...
async def economy_help(update, context):
    await update.message.reply_text(
        ...
        f"📜 /quests — квесты\n"
        f"🎁 /referral — кристаллы\n"
        f"🏢 /biz — бизнесы\n"
        f"🏦 /deposit — вклады\n"
        f"💱 /convert — обмен валют"
    )
...
async def casino_help(update, context):
    await update.message.reply_text(
        ...
        f"🃏 /blackjack сумма\n"
        f"🎲 /bet_dice сумма\n"
        f"⚽ /football_bet сумма\n"
        f"🏀 /basket_bet сумма\n"
        f"🎳 /bowling_bet сумма\n"
        f"🎯 /darts_bet сумма\n\n"
        f"{divider()}\n"
        f"Мин: футбол {FOOTBALL_MIN_BET}, баскет {BASKET_MIN_BET}, боулинг {BOWLING_MIN_BET}"
    )


async def rp_help(update, context):
    lines = [f"{frame('RP-ДЕЙСТВИЯ', '💞')}\n"]
    for k, d in RP_ACTIONS.items():
        emoji, act, _, aliases = d
        ru = ", ".join(aliases[:2]) if aliases else ""
        lines.append(f"{emoji} /{k} | {ru}")
    lines.append("")
    lines.append("💡 Работает: ответ на сообщение / @username")
    lines.append("🇷🇺 Или просто напиши: обнять @юзер")
    await update.message.reply_text("\n".join(lines))
...
async def love_help(update, context):
    await update.message.reply_text(
        ...
        f"💔 /divorce — развод\n"
        f"💞 /couple — пара\n"
        f"🎁 /marriage_gift сумма — прокачка\n"
...
TIMEZONES = {
    "москва": "Europe/Moscow", "мск": "Europe/Moscow", "киев": "Europe/Kiev",
    "минск": "Europe/Minsk", "алматы": "Asia/Almaty", "токио": "Asia/Tokyo",
    "лондон": "Europe/London", "париж": "Europe/Paris", "нью-йорк": "America/New_York",
    "лос-анджелес": "America/Los_Angeles", "пекин": "Asia/Shanghai",
    "дубай": "Asia/Dubai", "сидней": "Australia/Sydney",
}
...
async def time_cmd(update, context):
    if context.args:
        ...
        now = datetime.now(tz)
        await update.message.reply_text(f"🕐 {city.title()}: {now.strftime('%d.%m.%Y %H:%M:%S')}")
        return
    lines = [f"{frame('ВРЕМЯ В ГОРОДАХ', '🕐')}\n"]
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(url, timeout=aiohttp.ClientTimeout(total=8)) as resp:
                    ...
                    if not rate:
                        await update.message.reply_text("❌ Нет валюты.")
                        return
...
async def referral(update, context):
    ...
    await update.message.reply_text(
        f"{frame('КРИСТАЛЛЫ', '🎁')}\n\n"
        f"🔗 {link}\n\n"
        f"👥 Приглашено: {data['referral_count'] or 0}\n"
...
async def donate_info(update, context):
    await update.message.reply_text(
        f"{frame('ДОНАТ', '💠')}\n\n"
        f"⭐ VIP: {VIP_PRICE_DONAT} 💠\n"
        f"👑 Корона: {CROWN_PRICE_DONAT} 💠\n\n"
        f"💳 Для покупки обратитесь к админам:\n"
        f"• @kvails\n"
        f"• @notuwife\n\n"
        f"📊 /top_donat — топ"
    )
...
async def top_messages_day(update, context):
    ...
    medals = ["🥇", "🥈", "🥉"] + ["🔹"] * 7
    lines = [f"{frame('ТОП СООБЩЕНИЙ (сутки)', '💬')}\n"]
    for i, (uid, un, nk, fn, m) in enumerate(users):
        lines.append(f"{medals[i]} {_name_from_row(un, nk, fn, uid)} — {m or 0} 💬")
    await update.message.reply_text("\n".join(lines))
...
async def top_messages_week(update, context):
    ...
    medals = ["🥇", "🥈", "🥉"] + ["🔹"] * 7
    lines = [f"{frame('ТОП СООБЩЕНИЙ (неделя)', '💬')}\n"]
    for i, (uid, un, nk, fn, m) in enumerate(users):
        lines.append(f"{medals[i]} {_name_from_row(un, nk, fn, uid)} — {m or 0} 💬")
    await update.message.reply_text("\n".join(lines))
...
async def dice_game(update, context):
    ...
    if p > b:
        ...
        res = "🤝 Ничья"
    await update.message.reply_text(f"🎯 {user_mention(update.effective_user)}: {p}\n🤖 Бот: {b}\n{res}", parse_mode=ParseMode.MARKDOWN)
    check_quest(uid, "play_5")
...
async def bowling_free(update, context):
    msg = await update.message.reply_dice(emoji="🎳")
    s = msg.dice.value
    txt = "🎳 СТРАЙК!" if s == 6 else f"🎳 Сбито {s}"
    await update.message.reply_text(f"{user_mention(update.effective_user)} бросает!\n{txt}", parse_mode=ParseMode.MARKDOWN)
...
async def football_bet(update, context):
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("/football_bet сумма")
        return
    amount = int(context.args[0])
    uid = update.effective_user.id
    db.get_user(uid, update.effective_user.username, update.effective_user.first_name)
    if amount < FOOTBALL_MIN_BET or db.get_balance(uid) < amount:
        await update.message.reply_text(f"❌ Мин {FOOTBALL_MIN_BET}")
        return
...
async def basket_bet(update, context):
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("/basket_bet сумма")
        return
    amount = int(context.args[0])
    uid = update.effective_user.id
    db.get_user(uid, update.effective_user.username, update.effective_user.first_name)
    if amount < BASKET_MIN_BET or db.get_balance(uid) < amount:
        await update.message.reply_text(f"❌ Мин {BASKET_MIN_BET}")
        return
    db.add_balance(uid, -amount)
    msg = await update.message.reply_dice(emoji="🏀")
    s = msg.dice.value
...
async def bowling_bet(update, context):
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("/bowling_bet сумма")
        return
    amount = int(context.args[0])
    uid = update.effective_user.id
    db.get_user(uid, update.effective_user.username, update.effective_user.first_name)
    if amount < BOWLING_MIN_BET or db.get_balance(uid) < amount:
        await update.message.reply_text(f"❌ Мин {BOWLING_MIN_BET}")
        return
    db.add_balance(uid, -amount)
    msg = await update.message.reply_dice(emoji="🎳")
    s = msg.dice.value
...
async def darts_bet(update, context):
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("/darts_bet сумма")
        return
    amount = int(context.args[0])
    uid = update.effective_user.id
    db.get_user(uid, update.effective_user.username, update.effective_user.first_name)
    if amount < 20 or db.get_balance(uid) < amount:
        await update.message.reply_text("❌ Мин 20")
        return
    db.add_balance(uid, -amount)
    msg = await update.message.reply_dice(emoji="🎯")
    s = msg.dice.value
...
async def work(update, context):
    ...
    await update.message.reply_text(
        f"💼 {job}\n💵 +{earned}\n📈 Уровень: {new_lvl} (×{WORK_LEVEL_MULTIPLIER ** (new_lvl - 1):.2f})"
    )
...
async def pay(update, context):
    ...
    db.add_balance(uid, -amount)
    db.add_balance(tdata["user_id"], amount)
    await update.message.reply_text(f"💸 {user_mention(update.effective_user)} → {tdisp}: {amount}", parse_mode=ParseMode.MARKDOWN)


async def shop(update, context):
    lines = [f"{frame('МАГАЗИН', '🛒')}\n"]
    for iid, (name, price, desc) in SHOP_ITEMS.items():
        if iid == "vip":
            continue
        lines.append(f"{name}\n   {price} 💰 | {desc}\n   ID: {iid}")
    lines.append("\nКупить: /buy ID")
    lines.append("⭐ VIP — только за 💠 донат")
    await update.message.reply_text("\n".join(lines))
...
async def buy(update, context):
    ...
    if iid == "vip":
        await update.message.reply_text("⭐ VIP покупается только за 💠. /donate")
        return
    ...
    if gifts:
        ...
        for gk in gifts:
            if gk in GIFTS_COMMON:
                ...
                lines.append(f"• {GIFTS_LIMITED[gk][0]}")
    if len(lines) == 1:
...
async def rep_plus(update, context):
    ...
    await update.message.reply_text(f"⭐ +реп для {tdisp}\nРепа: {nr}", parse_mode=ParseMode.MARKDOWN)
...
async def rep_minus(update, context):
    ...
    if tdata["user_id"] == update.effective_user.id:
        await update.message.reply_text("Себе нельзя.")
        return
    nr = max(0, (tdata["reputation"] or 0) - 1)
    db.update_user(tdata["user_id"], reputation=nr)
    await update.message.reply_text(f"👎 -реп для {tdisp}\nРепа: {nr}", parse_mode=ParseMode.MARKDOWN)
...
async def profile(update, context):
    ...
    if data["warns"]:
        lines.append(f"⚠️ Варнов: {data['warns']}")
    if data["married_to"]:
        try:
            m = await context.bot.get_chat_member(update.effective_chat.id, data["married_to"])
            pdata = db.get_user(m.user.id)
            lines.append(f"💞 Партнёр: {display_name(pdata)}")
            lines.append(f"💍 Уровень брака: {data['marriage_level'] or 1} ({MARRIAGE_LEVELS.get(data['marriage_level'] or 1, '')})")
        except Exception:
            lines.append(f"💞 Партнёр: ID {data['married_to']}")
    if data["clan_id"]:
        c = db.get_clan(data["clan_id"])
        if c:
            lines.append(f"👑 Клан: {c[1]}")
    if data["bio"]:
        lines.append(f"📝 Био: {data['bio']}")
    if data["work_level"] and data["work_level"] > 1:
        lines.append(f"💼 Работа: {data['work_level']} lvl")
    if data["pinned_achievement"]:
        if data["pinned_achievement"] in ACHIEVEMENTS:
            lines.append(f"🏆 Закреплено: {ACHIEVEMENTS[data['pinned_achievement']][0]}")
    if got:
        lines.append(f"🎖 Ачивок: {len(got)}")
    if gifts:
...
async def title_cmd(update, context):
    ...
    data = db.get_user(uid)
    inv = (data["inventory"] or "").split(",")
    if "custom_title" not in inv and not data["is_vip"]:
...
async def marriage_gift(update, context):
    ...
    await update.message.reply_text(f"🎁 Уровень: {new_lvl}\n+{xp} XP")
    if new_lvl >= 5:
...
async def love_calc(update, context):
    ...
    await update.message.reply_text(f"💕 {user_mention(actor)} + {tdisp}\n{pct}% — {v}", parse_mode=ParseMode.MARKDOWN)
...
async def biz(update, context):
    if not context.args:
        lines = [f"{frame('БИЗНЕСЫ', '🏢')}\n"]
        for k, (name, price, income, cd) in BUSINESSES.items():
            lines.append(f"{name} — {price} 💰\n   Доход: {income} / {cd//3600}ч | ID: {k}")
        lines.append("\n/biz buy ID | /biz collect")
        await update.message.reply_text("\n".join(lines))
        return
    sub = context.args[0].lower()
    if sub == "buy" and len(context.args) >= 2:
        ...
        if total > 0:
            ...
            await update.message.reply_text("Нечего собирать.")
    else:
        await update.message.reply_text("/biz | /biz buy ID | /biz collect")


async def deposit_cmd(update, context):
    if not context.args:
        lines = [f"{frame('ВКЛАДЫ', '🏦')}\n"]
...
async def buy_gift(update, context):
    ...
    await update.message.reply_text("🎁 Куплен!")
...
async def remind(update, context):
    ...
    async def task():
        ...
        try:
            await context.bot.send_message(chat_id=cid, text=f"⏰ {um}, {text}", parse_mode=ParseMode.MARKDOWN)
        except Exception:
...
async def kick(update, context):
    ...
    try:
        ...
        await update.message.reply_text(f"👢 Кикнут.")
    except Exception as e:
        await update.message.reply_text(f"Ошибка: {e}")
...
async def sponsor_list(update, context):
    ...
    for sid, cid, title, url, reward in sponsors:
        ...
        if not done:
            kb.append([InlineKeyboardButton(f"Проверить: {title}", callback_data=f"sp_check:{sid}")])
    if kb:
        await update.message.reply_text("\n".join(lines), parse_mode=ParseMode.MARKDOWN,
...
async def sponsor_check_cb(update, context):
    ...
    try:
        ...
        await q.answer("Не могу проверить.", show_alert=True)
        return
    if db.mark_sponsor_done(sid, uid):
        db.add_balance(uid, reward)
        await q.answer(f"✅ +{reward} 💰", show_alert=True)
        await q.edit_message_text(f"✅ {title} — +{reward} 💰")
...
async def admin_help(update, context):
    ...
    text = (
        ...
        f"/admin_takebal @юзер сумма\n"
        f"/admin_setcrystals @юзер сумма\n"
        f"/admin_adddonat @юзер сумма\n"
        f"/admin_takedonat @юзер сумма\n\n"
        f"👤 ПОЛЬЗОВАТЕЛИ:\n"
        f"/admin_user @юзер\n"
        f"/admin_reset @юзер\n"
        f"/admin_ban @юзер\n"
        f"/admin_unban @юзер\n"
        f"/admin_freeze @юзер\n"
        f"/admin_unfreeze @юзер\n"
        f"/admin_unmarry @юзер\n"
        f"/admin_setname @юзер имя\n"
        f"/admin_giveitem @юзер ID\n"
        f"/admin_setvip @юзер 1/0\n"
        f"/admin_setcrown @юзер 1/0\n\n"
        f"🏆 АЧИВКИ:\n"
        f"/admin_makeach ключ | Название | Описание\n"
        f"/admin_giveach @юзер ключ\n"
        f"/admin_takeach @юзер ключ\n\n"
        f"📢 СПОНСОРЫ И ПРОМО:\n"
        f"/admin_sponsor @канал Название награда\n"
        f"/admin_promo КОД сумма активации часы\n\n"
        f"🌐 ЧАТЫ:\n"
        f"/admin_chats\n"
        f"/admin_broadcast текст\n"
        f"/admin_stats\n\n"
        f"⚙️ /admin_ping /admin_version"
    )
    await update.message.reply_text(text)
...
async def admin_setvip(update, context):
    ...
    if len(context.args) <= idx or context.args[idx] not in ("0", "1"):
        return
    db.update_user(data["user_id"], is_vip=int(context.args[idx]))
    await update.message.reply_text("✅")
...
async def admin_setcrown(update, context):
    ...
    if len(context.args) <= idx or context.args[idx] not in ("0", "1"):
        return
    db.update_user(data["user_id"], has_crown=int(context.args[idx]))
    await update.message.reply_text("✅")
...
async def admin_reset(update, context):
    ...
    await update.message.reply_text("🔄 Сброшен.")
...
async def admin_promo(update, context):
    ...
    if db.create_promo(code, reward, max_acts, hours):
        await update.message.reply_text(f"✅ {code.upper()}")
    else:
...
async def admin_chats(update, context):
    if not is_bot_admin(update.effective_user.id, update.effective_user.username):
        return
    chats = db.get_all_chats()
    await update.message.reply_text(f"💬 {len(chats)}\n" + "\n".join(str(c) for c in chats[:30]))
...
async def admin_version(update, context):
    if not is_bot_admin(update.effective_user.id, update.effective_user.username):
        return
    await update.message.reply_text("🤖 v9.0")
...
def run_bot():
    ...
    application.add_handler(CommandHandler("darts_bet", darts_bet))
    ...
    application.add_handler(
        MessageHandler(filters.TEXT & ~filters.COMMAND, ru_rp_handler),
        group=2
    )
